...
namespace Rivet {
  ...
  public:
    ...
    void init() {
      ...
      FinalState fs;
      WFinder wfinder_mu(fs, Cuts::abseta < 2.4 && Cuts::pT > 0*GeV, PID::MUON, 0*GeV, 1000000*GeV, 0*GeV, 0.1, LeptonOrigin::PROMPT, PhotonOrigin::NODECAY, MassVariable::MT);
      //WFinder wfinder_mu(fs, Cuts::abseta < 2.4 && Cuts::pT > 0*GeV, PID::MUON, 0*GeV, 1000000*GeV, 0*GeV, 0.1, LeptonOrigin::PROMPT, PhotonOrigin::NODECAY, MassVariable::MT);
    ...
    void analyze(const Event& event) {
      ...
      if (wfinder_mu.bosons().size() == 1) {
        ...
        for (const Jet& j : apply<FastJets>(event, "Jets").jetsByPt(Cuts::pT > 30*GeV && Cuts::absrap < 2.4)) {
