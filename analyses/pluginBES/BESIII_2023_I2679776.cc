...
namespace Rivet {
  ...
  class BESIII_2023_I2679776 : public Analysis {
  public:

    /// Constructor
    RIVET_DEFAULT_ANALYSIS_CTOR(BESIII_2023_I2679776);
    ...
    void analyze(const Event& event) {
      ...
      for (const Particle& p : apply<UnstableParticles>(event, "UFS").particles()) {
        _nDs->fill();
        if (isSemileptonicDecay(p, {PID::ETA, PID::ANTIMUON, PID::NU_MU})) {
  ...
  RIVET_DECLARE_PLUGIN(BESIII_2023_I2679776);
