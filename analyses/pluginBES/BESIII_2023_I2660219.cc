...
namespace Rivet {
  ...
  /// @brief  J/psi -> Xi0 Xibar0
  class BESIII_2023_I2660219 : public Analysis {
  public:

    /// Constructor
    RIVET_DEFAULT_ANALYSIS_CTOR(BESIII_2023_I2660219);
    ...
    void analyze(const Event& event) {
      ...
      double cProton = pProton.p3().unit().dot(axis1);
      ...
      const double cPbar = pPbar.p3().unit().dot(axis2);
      _h_cProton[1]->fill(cPbar);
      // moments
      const double T1 = sqr(sinX)*n1x*n2x+sqr(cosX)*n1z*n2z;
      const double T2 = -sinX*cosX*(n1x*n2z+n1z*n2x);
      const double T3 = -sinX*cosX*n1y;
      const double T4 = -sinX*cosX*n2y;
      const double T5 = n1z*n2z-sqr(sinX)*n1y*n2y;
      _h_T1->fill(cosX,T1);
      _h_T2->fill(cosX,T2);
      _h_T3->fill(cosX,T3);
      _h_T4->fill(cosX,T4);
  ...
  RIVET_DECLARE_PLUGIN(BESIII_2023_I2660219);
