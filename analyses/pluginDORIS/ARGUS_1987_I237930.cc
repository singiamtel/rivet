...
namespace Rivet {
  ...
  class ARGUS_1987_I237930 : public Analysis {
  public:

    /// Constructor
    RIVET_DEFAULT_ANALYSIS_CTOR(ARGUS_1987_I237930);
    ...
    void analyze(const Event& event) {
      ...
      for (unsigned int ix=0;ix<TAU.decaying().size();++ix) {
        ...
        double q = phad.mass();
        double mtau = TAU.decaying()[ix].mass();
        ...
        int ibatch = iomega == 0 ? 1 : 0;
        _h_spect[1]->fill(phad.mass(),sqr(mtau)*mtau/q/sqr(sqr(mtau)-sqr(q))/(sqr(mtau)+2.*sqr(q)));
        LorentzTransform boost = LorentzTransform::mkFrameTransformFromBeta(omega.mom().betaVec());
        Vector3 axis = boost.transform(pim[ibatch].mom());
        FourMomentum ppi0 = boost.transform(pi0.mom());
        FourMomentum ppip = boost.transform(pip.mom());
        Vector3 norm = ppi0.p3().cross(pip.p3()).unit();
        double cTheta = axis.dot(norm);
    ...
    /// @name Histograms
    /// @{
    Histo1DPtr _h_spect[2],_h_angle;
    CounterPtr _c;
  ...
  RIVET_DECLARE_PLUGIN(ARGUS_1987_I237930);
