...
You can run the container interactively, giving read/write access to your local directory using
\begin{footnotesize}
\begin{verbatim}
  docker container run -it -v $PWD:$PWD -w $PWD hepstore/rivet:X.Y.Z /bin/bash
\end{verbatim}
\end{footnotesize}
\noindent The tarball with the HepMC events used for this tutorial can be obtained as follows
\begin{footnotesize}
\begin{verbatim}
...
printed in full on the next page\footnote{The materials for this tutorial
can also be found in the
\href{https://gitlab.com/hepcedar/rivet/-/tree/release-4-0-x/doc/tutorials/lhc-basic-tutorial}{Rivet repo}.}.
...
in a separate directory, you can also tell Rivet where to look via the environment
...
The one-dimensional continuous histogram pointer type is called \verb|Histo1DPtr|.
...
Strictly speaking, \verb|Histo1DPtr| is just an alias for the more generic
\verb|BinnedHistoPtr<double>| where \texttt{double} is the C++ template
parameter used to specify the edge type along the continuous axis. For histograms
along discrete axes, such as multiplicty distributions or arbitrary
\texttt{string} labels, it might be preferable to book \verb|BinnedHistoPtr<int>|
or \verb|BinnedHistoPtr<string>| instead, respectively.
This also works in higher (axis-)dimensions by adding more type labels
as template arguments (e.g.\ \verb|BinnedHistoPtr<double,int>|).

The \verb|analyze(const Event& event)| function is executed once per event.
Currently, all it does is fill a histogram with the number
\verb|1.0|\footnote{The type of this argument should match the axis type specified
in the histogram constructor.}, which isn't all that useful. It will be up to you
to make it do something clever in the next section, but let's first look at the
final part of the skeleton.
...
For instance, the \verb|LeptonFinder| projection can be used to dress the bare lepton four-momentum
...
The \verb|LeptonFinder| constructor
...
to include all muons (\verb|JetMuons::ALL|) but to exclude all invisible particles (\verb|JetInvisibles::NONE|).
...
Try plotting this with the \verb|--with-variations| flag to the \verb|rivet-mkhtml| command.
...
The \verb|--with-variations| flag superimposes the available variation weights in a lighter
...
below the central value. Although there is an in-principle agreement on a standard for
how to label weights\footnote{See \href{https://arxiv.org/abs/2203.08230}{arXiv:2203.08230}.},
not all generators have yet implemented this standard for their latest public release.
While Rivet default behaviour is to be as inclusive as feasible when it comes to weight
variations, it also provides some functionality to filter out a specific subset of the weights.
In the following we will look at a few examples for how to deal with weights when using the
plotting scripts, but in principle similar options are also available to (de-)select weights
already at run time (cf. \verb|rivet --help| for the available flags).
